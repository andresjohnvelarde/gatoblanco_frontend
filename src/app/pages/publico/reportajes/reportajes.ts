...
@Component({
  ...
  templateUrl: './reportajes.html',
  styleUrl: './reportajes.css',
})
export class Reportajes {
  apiUrl = `${environment.apiUrl}`;
  // Simulación de 100 reportajes para tener 10 páginas (8 por página)
  todosLosReportajes: any[] = [];

  reportajesPaginados: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.totalPaginas = Math.ceil(this.todosLosReportajes.length / this.itemsPorPagina);

    this.actualizarPagina();
  }

  cargarReportajes() {
    this.publicacionesService.getPublicaciones('reportaje').subscribe({
      next: (data) => {
        // Transformamos los datos del backend al formato esperado por la vista
        this.todosLosReportajes = data.map(n => ({
          id: n.idpublicacion,
      ...
      error: (err) => {
        console.error('Error cargando reportajes:', err);
      }
    });
  }
  ...
  actualizarPagina() {
    ...
    this.reportajesPaginados = this.todosLosReportajes.slice(inicio, fin);
